...
Command parse_command(std::string_view input){
    ...
    std::string_view descriptor = input.substr(first,last==std::string_view::npos ? std::string_view::npos : last - first);

    Command command;
    //Now switch case for what the command is

    if(descriptor == "SET") command.type=CommandType::Set;
    else if(descriptor == "GET") command.type=CommandType::Get;
    else if(descriptor == "DEL") command.type=CommandType::Del;
    else command.type=CommandType::Unknown;
